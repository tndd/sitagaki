...
def depart_bar_to_peewee_table(bar: Bar, timeframe: Timeframe) -> TableBarAlpaca:
    """
    Bar:
        Domain -> Peewee Table
    """
...
def depart_bar_list_to_peewee_table(
        bars: List[Bar],
        timeframe: Timeframe
) -> List[TableBarAlpaca]:
    """
    Bar<List>:
        Domain -> Peewee Table
    """
    return [depart_bar_to_peewee_table(bar, timeframe) for bar in bars]
